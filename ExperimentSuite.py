...
class ExperimentSuite:
    ...
    def train_model(self, layers, tbCallBack, train_x, train_y, test_x, test_y, loss, activation, epoch):
        ...
        model.add(tf.keras.layers.Dense(layers[0], activation=activation, input_shape=(train_x.shape[1],) ) )
        for i in range(1,len(layers)):
            model.add(tf.keras.layers.Dense(layers[i], activation = activation)) 
        # Add output layer with softmax activation function
        model.add(tf.keras.layers.Dense(len(self.enc.get_feature_names()), activation = tf.nn.softmax ) ) 
        # Compile the model with given loss and metrics
        model.compile(loss=loss, metrics=metrics, optimizer=tf.keras.optimizers.Adam())
